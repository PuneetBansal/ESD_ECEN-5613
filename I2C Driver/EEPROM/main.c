/*
AIM	    : PROGRAM TO IMPLEMENT I2C DRIVER USING BIT BANGING IN 8051 AND SDCC
...
void write_byte()
{
    uint16_t add;
    uint8_t data_1;
    printf_tiny("\n\rEnter the address at which you want to write in hex(000-7FF)\n\r->");
    add=input_from_user();
    if(add>00 && add <=2047)                         //Checking whether address is between 0x00-0xff
    {
    printf_tiny("\n\rEnter the data which you want to write\n\r->");
    data_1=input_from_user();

    /*Preparing the byte---------|1|0|1|0|A2|A1|A0|0|--------  */
    byte1=((0xA0)|((add & 0x700)>>7)| (0x00));

    /*Obtaining the least significant 8bits of the address*/
    byte2=(add & 0xff);

    start();
    write(byte1);   //Write the first control byte
    /*----------Waiting for Ack-----------------*/
    SCL=1;
    delay();
    delay();
...
printf_tiny("\n\rEnter the address from which you want to read the data. Please enter in hex(000-7FF)\n\r->");
add=input_from_user();
if(add>00 && add <=2047)                //Checking if the entered address is between 0X000-0X7ff
{
/*Preparing the byte---------|1|0|1|0|A2|A1|A0|0|--------  */
byte1=((0xA0)|((add & 0x700)>>7)| (0x00));
/*Obtaining the least significant 8bits of the address*/
